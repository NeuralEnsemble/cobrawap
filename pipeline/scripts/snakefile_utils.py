...
def read_stage_output(stage, working_dir, config_name, output_namespace="STAGE_OUTPUT"):
    with open(os.path.join(working_dir, stage, config_name), 'r') as f:
...
def create_temp_configs(stages, working_dir, config_name, temp_name='temp_config.yaml'):
    for i, stage in enumerate(stages):
        config_path = os.path.join(working_dir, stage, config_name)
        new_config_path = os.path.join(working_dir, stage, temp_name)
